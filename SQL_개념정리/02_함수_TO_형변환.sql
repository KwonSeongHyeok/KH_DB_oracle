...
SELECT 1234, TO_CHAR(1234, '99999999') FROM DUAL; --  '    1234' 앞에 공백이 4칸 생성
-- 9를 사용하지 않으면 기본으로 왼쪽 정렬 9의 반대는 아무것도 작성하지 않는 것

SELECT 1234, TO_CHAR(1234, '00000000') FROM DUAL; --  '    1234' 앞에 0이 4칸 생성

-- 숫자 -> 문자열 변환 시 주의사항
--> 지정된 칸 수가 숫자 길이보다 적으면 전부 # 변환돼서 출력
SELECT 1234, TO_CHAR(1234, '000') FROM DUAL;
-- 지정된 자리값보다 변환해야하는 값의 길이가 더 크기 떄문에 #### 출력
...
-- DUAL 테이블 사용해서 현재 날짜(SYSDATE) 2024년 06월 05일 오전 00시 00분 00초
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일" AM HH"시" MI"분" SS"초"') FROM DUAL;
...
-- 숫자 -> 날짜
SELECT TO_DATE(20240605, 'YYYYMMDD') FROM DUAL; --> 24/05/27
...
-- 50 미만 : Y, R 둘다 누락된 연도 앞부분 현재 2000년대 세기 추가
