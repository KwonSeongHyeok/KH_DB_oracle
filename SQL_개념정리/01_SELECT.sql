 ...
 *(asterisk) : 모든 , 포함하다를 나타내는

*/

-- EMPLOYEE테이블에서
-- 모든행의 사번(EMP_ID), 이름(EMP_NAME), 급여(SALARY) 조회
...
-- SYSTIMETAMP : 현재시간  (년, 월, 일, 시, 분, 초, MS + 지역(local)) 조회


회원가입할 때 DB에 들어오는 현재 시간을 기록할 때 생서
...
TO_DATE('문자열', '패턴')
예를 뜰어 2024-05-01 자 이후 회원가입 한 회원 조회
...
-- EMPLOYEE 테이블에서 이름, 이메일 이메일ㅇ @ 포함이라는 별칭
-- 1) 컬럼명 AS "별칭"
...
-- EMPLOYRR 테이블에서 사원이 존재하는 부서코드만 조회
...
/*
WHERE OR 사용 값1, 값2, 값3, ...과 같은 표시를 진행하기도 함
...
/**** IN 사용 ****/
-- EMPLOYEE 테이블에서 부서코드가 'D5', 'D6', 'D9' 아닌 사원의 이름, 부서코드, 급여 조회
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
SELECT EMP_ID, EMP_NAME FROM EMPLOYEE WHERE EMP_NAME LIKE '전%' AND EMP_NAME LIKE '%돈';
...
---> 나타난 정보를 오름차순으로 정렬할지 내림차순을 정렬할지
...
SELECT 문의 조회결과 (RESULT SET)를 정렬할때 사용하는 구문
...
FROM EMPLOYEE
WHERE DEPT_CODE IN ('D5', 'D6', 'D9')
ORDER BY DEPT_CODE;  -- ASC가 기본값이기 때문에 오름차순 생략 가능
...
-- 만약에 EMP_ID 로 ORDER BY에서 정렬하길 원한다면 1을 작성
-- 만약에 SALARY 로 ORDER BY에서 정렬하길 원한다면 3을 작성
