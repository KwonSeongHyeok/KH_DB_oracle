...
CREATE TABLE USER_MEMBER(USER_ID VARCHAR2(20),
                                        USER_PW VARCHAR2(20),
...
-- 제약조건
-- UNIQUE PRIMARY KEU FOREIGN KEY CHECK 
...
CREATE TABLE UNIQUE_TABLE(
    ...
    -- (방법2)
    USER_NAME VARCHAR(30), -- 테이블 레벨로 UNIQUE 제약 조건 설정
    -- 테이블 레벨 --
    -- UNIQUE(USER_NAME)
    CONSTRAINT USER_NAME_U UNIQUE (USER_NAME));

-- KH_CAFE 테이블 만들기
-- CAFE_ID NUMBER(10,0) 기본키(PRIMARY KEY)
-- CAFE_NAME VARCHAR2(100) NOT NULL 널값 못들어가게 설정
-- CAFE_PHONE VARCHAR2(2) 핸드폰 번호 카페 중복 X(방법1)
-- CAFE_ADDRESS VARCHAR(100) 주소 카페 중복 X(방법2)
...
-- UNIQUE는 컬럼에서 NULL이 한개는 허용됨 두 개 이상 안됨
-- PRIMARY KEY NULL 값 안되고 중복안됨


/*
-- 4. FOERIGN KEY(외부키) 제약조건

-- 배달 어플    - 카페 참조

참조된 다른 테이블의 컬럼이 제공하는 값만 사용할 수 있음
FOERIGN KEY - 삭제, 테이블 부모까지 삭제하는데 있어 에러발생
...
-- USER_GRADE GEADE_CODE 참조하는 테이블 생성
...
--> 핸드폰 번호를 기입할 때는 VARCHAR2를 사용해서 010모두 들어갈 수 있도록 설정
SELECT * FROM USER_GRADE;
...
-- 앞으로 GENDER 컬럼에는 남이나 여라는 글자만 들어갈 수 있음
...
제약조건으로 남이나 여만 가능하게 했는데 남자라는 조건이 들어와서 에러 발생
*/

INSERT INTO user_check VALUES(2, 'user02', 'pw02', '박철수', '여');

INSERT INTO user_check VALUES(2, 'user03', 'pw03', '강영희', '여');
...
-- 2번 방법 따로 index에 기록한 다음 조건 설정 한 경우 (한 줄 작성)
컬럼명 자료형 CONSTRAINT 인덱스에 기록할 이름 CHECK(컬럼명 IN('조건1', '조건2'))

-- 3번 방법 따로 index에 기록한 다음 조건 설정 한 경우 (여러 줄 작성)
컬럼명 자료형,

CONSTRAINT 인덱스에 기록할 이름 CHECK(컬럼명 IN('조건1', '조건2'))
