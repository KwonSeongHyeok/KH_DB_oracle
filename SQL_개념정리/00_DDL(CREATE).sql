...
VARCHAR2 : 최신버전 제일 많이 사용
    ...
    예를 들어 CHAR(10BYRE) VARCHAR(10BYTE)
...
/*
PRIMARY KEY (기본키)
 - 테이블에서 한 행의 정보를 찾기 위해 사용할 컬럼을 읨
 - NOT NULL + UNIQUE (한 행에서 COLUMN에서 똑같은 모양의 데이터는 존재해서는 안됨)
...
특정한 컬럼만 선택해서 사본 만들기
CREATE TABLE 새로운 테이블 이름 AS SELECT 컬럼명1, 컬럼명2, ... FROM 복사할 테이블 이름;
